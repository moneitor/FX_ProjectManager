...
temp_folder = r"C:\Users\mo_no\Documents\HERNAN\TUTORIALS\PYTHON\Practical_Python_for_VFX\exrs"
...
def make_directory(root, folders):
	"""
	Make directories from a list inside the root directory
	"""
	path =  os.path.normpath(os.path.join(root, folders))
...
def make_nested_directory(root, folders):
	"""
	Makes nested directories inside the root based in the folders dictionary
	"""
	for key, value in folders.items():
		...
		if not os.path.exists(full_path):
			...
			print(full_path)
			if type(value) == dict:			
				make_nested_directory(os.path.join(root, key), value)
